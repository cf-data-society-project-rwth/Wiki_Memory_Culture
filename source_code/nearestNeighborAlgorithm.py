...
fout = open("new_nearestneighbor_result.csv", "w", encoding="utf8")
...
for line in f:
    ...
    if resp.status_code == 200:
        ...
        if len(myarticle.visits) > 1:
            ...
            # algorithm to check and get the peak views of each page based on nearest neighbors
            # ref: https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.find_peaks.html
            newArray = np.array(myarticle.visits);
            maxView = np.amax(newArray)
            minView = np.amin(newArray)
            rangeLimit = np.arange(minView, maxView)
            p = np.percentile(rangeLimit, 50)
            peaks, _ = find_peaks(myarticle.visits, height=p)
            n = len(peaks)
            noCheckAgain = False

            while i < 7 + len(myarticle.visits) and noCheckAgain is False:
                currentIndex = i-7
                lol = 0
                while lol < n:
                    if currentIndex == peaks[lol]:
                        print('index peak '+str(currentIndex))
                        checkYearly = False
                        ...
                        if checkYearly is True or len(myarticle.monthList) == 0:
                            ...
                            mylist.append(myarticle.visits[currentIndex])
                            noOfPeaks += 1
                        else:
                            noCheckAgain = True
                            noOfPeaks = 0
            ...
            # checking if the peak is a yearly event or not
            if 2 < noOfPeaks <= 4:
                ...
                while j < noOfPeaks:
                    ...
                    print("percentage of peak" + str(j) + "is" + str(percentage) + "%")
